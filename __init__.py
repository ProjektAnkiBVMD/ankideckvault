...
class DataWindow(QDialog):
    ...
    def createAnkiAddon(self):
        self.saveData()
        addon_name = self.addon_name_input.text() if self.addon_name_input else "Default"
        try:
            ...
            # Create manifest.json
            meta_data = {"name": f"Decklink Collection: {addon_name}", "mod": int(time.time()), "min_point_version": 1}
            ...
            with zipfile.ZipFile(addon_output_path, 'w') as zipf:
                for root, dirs, files in os.walk(addon_files_dir):
                    ...
                    for file in files:
                        ...
                        zipf.write(file_path, os.path.relpath(file_path, addon_files_dir))
            ...
            QMessageBox.information(self, "Success", f".ankiaddon file created at {addon_output_path} \n A window will open a folder with 'output.ankiaddon' this is your Addon File")
    ...
    def addAddonName(self):
        ...
        self.addon_name_input = QLineEdit(self)  # Use self to make it accessible elsewhere
    ...
    def checkboxStateChanged2(self, state):
        ...
        if state == 2:
            adddownloadto = True
            showInfo('Download To Location Button will now be integrated to your Addon!')
        else:
            adddownloadto = False
    ...
    def saveData(self):
        ...
        if addpwd_inp is True:
            passphrase, ok = QInputDialog.getText(self, "Passphrase", "Enter passphrase for encryption:")

            if not ok or not passphrase:
                QMessageBox.warning(self, "No Passphrase", "No passphrase provided. Aborting encryption.")
                return
        ...
        with open(str(json_path), 'w', encoding='utf-8') as f:
            f.write(data_json)
        QMessageBox.information(self, "Decklinks Saved",
